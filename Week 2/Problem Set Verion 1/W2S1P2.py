...
As part of the application, users will be able to easily search their favorite artist 
and find out the day, time, and stage the artist is playing at. 
Write a function get_artist_info() that accepts a string artist 
and a dictionary festival_schedule mapping artist's names to dictionaries 
containing the day, time, and stage they are playing on. 
...
edge cases:
>> if the artist searched for does not exist, 
return the dictionary {"message": "Artist not found"}.

PLAN:
1. check if the artist is in the dictionary 
    1.1 if yes, return the schedule
    1.2 if no, return {"message": "Artist not found"}
"""
